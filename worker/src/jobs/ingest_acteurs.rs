...
async fn fetch_person_ids(pool: &PgPool) -> sqlx::Result<HashMap<String, i64>> {
    ...
    // pas) : cette fonction ne sert qu'à réconcilier les acteurs AMO30, tous porteurs d'un
    // `an_uid`, d'où le filtre explicite plutôt qu'un `Option` propagé jusqu'aux appelants.
    let rows =
